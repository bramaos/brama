...
- Never write to `os.Stdout` or call `fmt.Print*`; write to `env.Out` or `env.Err` only for human-only extras that return early on `env.JSON`, like `writeSkeletonPreview`.
