...
- Keep `RunE` to reading flags, args and the working directory, then calling `run<Command>` with everything as parameters, fakeable dependencies included.
- Pass `cmd.Context()` to `run<Command>` as its first parameter; wrap it (`signal.NotifyContext`) rather than start from `context.Background()`.
